...
for i = 1:numel(road_classes)
    ...
    suspension_all.(className) = [];
    radar_all.(className) = [];

    for iter = 1:num_iterations
        fprintf('Running Class %s, Iteration %d....\n', className, iter);

        eval(sprintf('[road_profile_time, road_profile_height] = generate_iso8608_profile(''%s'', 2, 10, 100);', className));
